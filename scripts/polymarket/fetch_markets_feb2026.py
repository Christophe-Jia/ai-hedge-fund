"""
Fetch Polymarket geopolitical market list and hourly price history for Feb 2026.
...
Outputs:
  data/polymarket/markets_feb2026.json                    — market metadata
  data/polymarket/price_history_feb2026/<conditionId>.json — hourly YES probability per market

Usage:
  poetry run python scripts/polymarket/fetch_markets_feb2026.py
"""
...
PRICE_DIR = DATA_DIR / "price_history_feb2026"

# Price history window: Feb 20 – Mar 5 2026 (captures run-up + aftermath)
START_DT = datetime(2026, 2, 20)
END_DT = datetime(2026, 3, 5)
START_TS = int(START_DT.timestamp())
END_TS = int(END_DT.timestamp())

# Markets closing in this range (wide net)
MARKET_START_DATE = "2026-02-01"
MARKET_END_DATE = "2026-03-07"
...
def fetch_markets() -> list[dict]:
    """Fetch all markets closing in the Feb 2026 window, then filter by keyword."""
    print("=== Step 1: Fetching Polymarket Feb 2026 markets via Gamma API ===")
    ...
    print(f"  → {len(all_markets)} unique markets in window")
...
def main() -> None:
    ...
    # save metadata
    out_path = DATA_DIR / "markets_feb2026.json"
    ...
    if markets:
        ...
        print("\nDone. Price histories saved to data/polymarket/price_history_feb2026/")
    else:
        print("\n[!] No markets found.")
