...
**纠正方式是 append-only，不是改历史**：错误留在 `43913a8f`，更正写在后续 commit 的 message 里——**具体是 `a1e0f6ca` 的 message**，而**不是**本小节所在的 `03d3204b`（那里只有 §1.6 的顺序不变量与符号层修正，message 里没有归属更正）；可 grep 的说明放进仓库（即本小节 §1.6.2）。**这里必须点明 hash 而不能写「本条即是」**——因为「本条」会被读者理解成"上一个碰这个文件的 commit"，2026-09-20 确实就这么被误读了（读者查 `03d3204b` 的 message 找不到更正，据此判定更正不存在）。**指向记录时给出精确坐标，是本节全部内容的实践。****任何"某人没写过某文件"的断言，都必须能给出证据**（`git log -- <file>` 的提交者集合、或该 agent 的自述）；给不出就不要写进 commit message。
